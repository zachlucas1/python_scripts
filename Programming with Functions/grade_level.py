...
def grade_level_counts(df):
    ...
    #Reorders the data frame by dictionary rather than value count
    counts = counts.reindex(level_dict.values(), copy=False)

    #drops rows that have no count
    counts.dropna(inplace=True)

    #changes them to integers because they sometimes get changed to floats in the process
